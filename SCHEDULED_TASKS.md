...
The warning `Policy migration missing: new bets remain untagged` is separate from pipeline execution failures. [supabase/migrate-paper-betting-policy.sql](supabase/migrate-paper-betting-policy.sql) only adds a nullable audit column and an index; it does not change bankroll, staking, selection rules, or historical tags. Applied September 22, 2026 with user approval through the authenticated Supabase SQL Editor. Verified `policy_version` is nullable text, `idx_paper_bets_policy` indexes `(account_id, policy_version, placed_at)`, and the application API can select the column. Future internal auto-bets can now record their selection policy; existing bets remain untagged. No betting run was triggered to verify the schema.

Verified September 22, 2026: the registered `RacingPredictor-DailySync` task was manually triggered at 20:36 local time and finished at 20:55 with `LastTaskResult = 0`. All steps succeeded: 184 races/2,769 entries ingested, 390 races/4,587 results refreshed, forecasts generated for 166 races, retrospective backfill already current, 36,447 forecasts backtested across 14 models, calibration refreshed from 3,936 races, auto-betting and settlement completed, 46 odds-feed races processed without failures, and stale data pruned. All eight page snapshots published. No bets were placed in this run; internal races were outside the betting window. This verifies the registered action and launcher, not an unattended hourly trigger or an in-window betting run. Missing-probability rejection is covered by regression tests; this particular live feed contained no unscorable runners.